...
Ball lifecycle and the per-state physics rules live in [`02-ball-lifecycle.md`](../ball/INDEX.md). This doc covers only the court-control mechanics that drive transitions: the friendship-bound crossing, the side-band miss, and the apex return.
...
The miss transitions the ball PLAY → OUT-REST; the state-transition handling itself lives in [`02-ball-lifecycle.md`](../ball/INDEX.md).
