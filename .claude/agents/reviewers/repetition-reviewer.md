...
- Reviewer posture and verdict shape: `.claude/skills/reviewers/SKILL.md`
...
Per `.claude/skills/reviewers/SKILL.md`. Approve is silent (label only). Block posts inline review comments anchored to `path:line`, never on the main PR thread. Each finding names the duplicate location or the missing destination so the author can fix it without searching.
