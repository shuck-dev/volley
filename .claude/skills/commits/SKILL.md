...
**Do NOT put the `#N` in the PR body or commit body at all, bare or otherwise.** A closing verb (`closes`/`fixes`/`resolves #N`) fires GitHub's own issue-close on merge, which drags the linked Linear issue to Closed; that is the live hazard. A bare `#N` does not auto-close, but the body still needs no number: the branch name carries it and drives Linear movement, and keeping numbers out of the body entirely removes the habit that produces the dangerous verb form in the first place. The Linear link is made by hand after the PR opens (see below). See [`designs/ai/lane-semantics.md`](../../../designs/ai/lane-semantics.md).
