...
5. **Open the challenge as a draft** (`gh pr create --draft`). Every challenge opens draft and stays draft; Josh flips it ready when he is choosing to merge, never the dispatcher (a synthesis APPROVE means verified, not ready). Do not enable auto-merge, do not apply approval labels; the maintainer merges by hand. Review is a dispatcher spot-check by default (read the diff, run the suite, verify the behaviour); do NOT fan reviewers automatically. A reviewer battle fires only when Josh asks for one; when he does, run it per [`battle`](../battle/SKILL.md).
