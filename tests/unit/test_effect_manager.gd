...
func _make_always_modify_stat_effect(
	stat_key: StringName, operation: StatModifier.Operation, value: float
) -> Effect:
...
func test_register_source_applies_always_modify_stat_effect() -> void:
	var effect := _make_always_modify_stat_effect(&"paddle_speed", StatModifier.Operation.ADD, 50.0)
...
func test_register_source_applies_multiply_effect() -> void:
	var effect := _make_always_modify_stat_effect(
		&"paddle_speed", StatModifier.Operation.MULTIPLY, 2.0
	)
	var item := _make_item("test_item", [effect])
...
func test_register_source_applies_multiple_effects() -> void:
	var effect_a := _make_always_modify_stat_effect(
		&"paddle_speed", StatModifier.Operation.ADD, 20.0
	)
	var effect_b := _make_always_modify_stat_effect(
		&"paddle_size", StatModifier.Operation.ADD, 10.0
	)
...
func test_multiple_items_stack_modifiers() -> void:
	...
	var effect_b := _make_always_modify_stat_effect(
		&"paddle_speed", StatModifier.Operation.ADD, 30.0
	)
