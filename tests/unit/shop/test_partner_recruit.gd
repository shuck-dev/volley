...
class TestPartnerRecruit:
	...
	func test_cannot_recruit_below_threshold() -> void:
		_ball_manager.add_soul(_martha.unlock_threshold - 1)
		assert_false(_progression_manager.can_recruit_partner(&"martha"))

	func test_cannot_recruit_without_enough_balance() -> void:
		_ball_manager.add_soul(_martha.unlock_threshold)
		_ball_manager.subtract_soul(_martha.unlock_threshold - _martha.unlock_cost + 1)
		assert_false(_progression_manager.can_recruit_partner(&"martha"))
	...
	func test_recruit_deducts_cost() -> void:
		_ball_manager.add_soul(_martha.unlock_cost + 50)
		_progression_manager.recruit_partner(&"martha")
		assert_eq(_ball_manager.get_soul_balance(), 50)

	func test_recruit_sets_active_partner() -> void:
		_ball_manager.add_soul(_martha.unlock_cost)
		_progression_manager.recruit_partner(&"martha")
	...
	func test_recruit_returns_false_when_already_unlocked() -> void:
		_ball_manager.add_soul(_martha.unlock_cost * 2)
		_progression_manager.recruit_partner(&"martha")
...
class TestPartnerRecruitPersistence:
	...
	func test_deferred_recruit_available_emitted_for_threshold_met_save() -> void:
		...
		ball_manager.economy.total_soul_earned = 200
		...
		var progression_manager: Node = ProgressionManagerFactory.create_manager(
			self, ball_manager, {"partners": partners}
		)
