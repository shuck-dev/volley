...
func test_advancing_past_hold_duration_shifts_to_double() -> void:
	var shift := _make_shift(1.0, 1.0)
	shift.advance(1.0)
	assert_almost_eq(shift.get_offset(), 100.0, 0.0001, "normal -> double is +100% of range")


func test_shifting_cycles_double_to_half() -> void:
	var shift := _make_shift(1.0, 1.0)
	shift.advance(1.0)
	shift.advance(1.0)
...
func test_shifting_wraps_half_back_to_normal() -> void:
	var shift := _make_shift(1.0, 1.0)
	shift.advance(1.0)
	shift.advance(1.0)
	shift.advance(1.0)
...
func test_hold_duration_respects_configured_range() -> void:
	for i in 20:
		var shift := _make_shift(2.0, 5.0)
		shift.advance(1.999)
		assert_eq(shift.get_offset(), 0.0, "hold duration should never be below min_interval")
