...
| `test_ball_regime_transitions.gd` | SH-245 (Scenario 7), SH-247 (Scenario 8), SH-262 (Scenario 9) all drive `Area2D.input_event` for press and `_drag._input` for release. | Scenarios 1–6 use `grab_from_rack`/`grab_live_ball` + `attempt_release`. | Scenarios 1–6 isolate placement state, court-edge clamping, save round-trip, and temporary-ball semantics. The press-drag-release path itself is verified end-to-end by Scenarios 7–9. |
