# The Cliff, the Gate, the Postgame

The ending and what comes after. The cliff is the chosen-acceptance bookend to the break; the gate is the one beat the whole game has been reaching for; the credits play over the rally that proves it; postgame keeps the rally going. High-level architecture in `00-two-styles.md`. Part 2's arc into the gate is in `03-reconstruction.md`.

## The cliff and the gate

The structural shape: items invoke memories, the final memory yields the key. The protagonist unlocks the garden gate and steps through to the cliff in Reality. Zach is on the memorial bench dedicated to their friend.

Full canon in [`../narrative/outline.md`](../narrative/outline.md). The split-frame staging at the bench lives in [`../art/bible.md`](../art/bible.md) § 9.

## Credits

Credits play over a rally between the protagonist and Zach. Construction's saturated light returns with the texture Reality has worn into it across Part 2.

Audio per the SH-281 arc: full-orchestra range, the synth warmth that opened the game returning layered with the acoustic instruments Reality brought. Audio synthesis at exactly the visual moment of synthesis.

## Postgame

Zach is found. He can now share the construct with the protagonist, knowing and accepting two truths: that the construct is not real, and that Fern is dead. The game continues, new items, venues etc. Full canon in [`../narrative/outline.md`](../narrative/outline.md).

The cliff stays returnable through the now-unlocked gate. The cliff is a place to sit, look at the sea, listen. No new gameplay there. Grief integrated as access, not as event.

## Open questions

- **Reality cycling post-gate.** After the gate, does the game continue, close, or open into a Regret-style alt-perspective? Default: stays open with the cliff as the persistent affordance.
- **What Construction looks like in postgame.** With Zach sharing the construct, what is the relationship between rallies with him and the rest of the partners?
- **Zach after the gate.** Reachable in Reality continuously? A character the player can visit at the shop or on the bench?
- **Save state and the gate.** Does reaching the gate create a save-flag the player crosses once, or is the moment replayable?
