...
**The issue number lives in the branch name only, never in the PR body or commits.** The branch is `feature/<gh-number>-<slug>`; that is where the GitHub number belongs and the only place it appears. The PR body and commit bodies carry no issue number at all. Any number in the body creates a GitHub linked-issue, and a GitHub action-verb ahead of it (`closes #123`) fires GitHub's own issue-close on merge, which drags the linked Linear issue all the way to Closed, falsely reading as released. Even a bare `#123` in the body is an unwanted link; the branch name drives Linear movement, and the Linear attachment link is made by hand after the PR opens.
