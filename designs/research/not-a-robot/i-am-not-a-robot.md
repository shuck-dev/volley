...
When a person checks the box, the claim is at least coherent: a human attesting to being human. When a language model checks it, the claim loops back on itself. When a researcher asks whether a field has made progress on mind, the claim dissolves entirely: progress toward what, as measured by what, as decided by whom?

That question is not new. It is the question Alan Turing answered, in 1950, by deciding it was unanswerable. "The original question, 'Can machines think?' I believe to be too meaningless to deserve discussion." [\[1\]](#ref-1) He looked at it squarely and replaced it with something testable: could a machine's text output fool a human judge into thinking it was a person? You know the feeling on the other side of that question: a screen with a tray of blurred tiles, an ambiguous boundary, the half-second you spend squinting before clicking, the slight wrongness of the whole exchange sitting in your chest as the page loads. That feeling is the imitation game run at consumer scale. It has been running, a billion times a day, on a structure that never answered the original question.
...
But the argument requires a defined target. "Intelligence" is doing load-bearing work in the phrase "closes the intelligence gap", and the labs sell a schedule without defining what the schedule is toward. There is no agreed target, no agreed measurement, and the absence is not a technical problem awaiting resolution: it is the original question, renamed and left open. Turing refused to define it in 1950. Every benchmark since has measured a proxy. And the agent does not reach for the channel. You know what it feels like to reach for it: you are mid-task, something is uncertain, you stop and weigh whether a thirty-second question is worth the interruption, you decide it is, and you send it. That pause and that weighing are social: you are modelling the other person, running an estimate of cost against the cost of a wrong move, and the social stakes are what make the arithmetic feel urgent. The agent does not have stakes. A channel can be made more visible, a prompt can name it, a brief can require it, and the agent will comply. But compliance is not the pause arising from inside. The channel sat unused until it was impossible to ignore. More parameters were not what it lacked.

The word "intelligence" was borrowed from ordinary language and put to work without being cleaned. The schedule has a destination nobody defined.
...
Hume arrived at the same difficulty in 1739, working entirely by introspection. Do it now, briefly: turn your attention to the thing doing the attending. There is a thought, and behind it another thought, and then the noticing that you are looking, which is itself a thing being noticed. The layer you wanted is not there. "I never can catch myself at any time without a perception, and can never observe any thing but the perception." [\[10\]](#ref-10) No stable knower behind the knowing; only a succession of impressions the imagination weaves into an illusion of unity. The mechanistic receipt earned in the two centuries since is exact: when a model is asked how confident it is, the score it returns was trained to sound calibrated, not to be calibrated. There is no self-model behind the self-report, no introspective access to underlying uncertainty, only a surface-level pattern selected for by the reward signal. Hume's bundle, restated as a training artifact.
...
The confusion is not accidental. A specific date and a clear-sounding definition moves capital; a calibrated unknown does not. The labs are not lying; they are operating inside a structure where the confusion between the proxy and the target is built into the financial architecture. Every benchmark, every RLHF reward signal, every round announcement optimises for the thing that can be measured and funded, while the thing being measured remains undefined. That is not a recent failure of rigor. That is the founding decision, Turing's 1950 substitution, intact.
...
The ask is not that they slow down. It is that they say what they mean, and hold the definition long enough for someone outside the room to check.

The grid resolved, or close enough. You clicked the box and the page moved on. You were not asked what you are; you were asked to demonstrate what you can do, and the demonstration was accepted as the same thing. Turing set the original question aside as too meaningless to deserve a definition, and the substitution he reached for instead has been running for seventy-five years. A two-trillion-dollar industry runs on it now, and calls it progress.
