...
2. Player drags from `GearRack` onto the main character to equip, or from the character back to `GearRack` to deactivate. Equipment interaction is a timeout-only verb; presses on equipped art outside the timeout window are refused.
