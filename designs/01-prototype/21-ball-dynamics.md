...
Ball speed is friendship energy. It carries through grab-and-release: the magnitude the rally had built up before the grab is the magnitude the released ball resumes at, with the gesture choosing only the direction. It resets only on miss. A mid-rally grab is a redirect, not a reset; the streak's accumulated speed survives the held-token detour.
