...
ProgressionData is a data bag: it holds state and serialises it. It does not watch signals or make decisions. SaveManager handles disk I/O. ItemManager handles items, effects, and friendship balance. None of these should own "when the player crosses 50 friendship, unlock the clearance," because that is a progression milestone, not an item concern or a persistence concern.
...
- **Unit:** Clearance unlocks when friendship reaches threshold
