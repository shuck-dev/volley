...
**PR 4b: the controller becomes an autoload.** `ItemDragController` moves out of the scene tree into `project.godot`. The controller reads the cursor through its own `get_canvas_transform()` and falls back to its own `global_position`, and an autoload sits at the tree root rather than inside the loaded scene, so cursor positions resolve against the root's canvas space. Settling what a cursor position means for a node outside the scene is this PR's substance.
