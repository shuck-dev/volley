...
Mid-rally, equipment interaction is not a player verb. The press on an equipped item's mounted art is refused by `BallDragController.grab_equipped_from_character` when a rally is in progress (`TimeoutController.IDLE` and at least one ball in `PLAY_NORMAL` / `PLAY_ARC`). The gesture does not begin; nothing to drop, nothing to deactivate. The shared check lives on `RallyGate.is_rally_in_progress(timeout_controller, reconciler)`, invoked with injected refs at every callsite. The dev panel's per-item remove-level button uses the same gate so debug paths cannot desync the effect system during play.
