...
Mid-rally, the press on an equipped item's mounted art is refused at the entry. `BallDragController.grab_equipped_from_character` returns false while a rally is in progress (`TimeoutController.IDLE` and at least one ball in `PLAY_NORMAL` / `PLAY_ARC`); the gesture never starts, nothing is held, nothing deactivates. The shared check lives on `RallyGate.is_rally_in_progress(timeout_controller, reconciler)`, invoked with injected refs at every callsite. The dev panel's per-item remove-level button uses the same gate so debug paths cannot desync the effect system during play.
