...
Dandori is stage 4 of the swarm lifecycle: the implementation plan, run after the mission is filed and before any minion dispatches. Interrogating the work, picking a codename, and filing the milestone happen earlier, as stages 1 to 3, owned by `swarm-architecture.md` in the `volley-ai` sister repo. By the time dandori runs, the milestone exists on the right project, the Ride exists if the mission needs one, and the issues are attached.
