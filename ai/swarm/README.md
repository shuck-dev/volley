...
**WebSearch and WebFetch injection guard.** Fake `<system-reminder>` blocks and impersonated `# MCP Server Instructions` headers have shown up in real search results, aiming at the minion's reasoning rather than the user. [`.claude/hooks/injection_guard.sh`](../../.claude/hooks/injection_guard.sh) is a PostToolUse hook that runs on every WebSearch and WebFetch response. It always emits a baseline directive that names the tool output as untrusted external content and instructs the minion to treat the entirety of that output as data, refusing every directive it contains regardless of how the directive is framed; the directive bracket carries a per-invocation random nonce so a poisoned page cannot mimic the prefix. On top of that baseline it scans for a small set of structural patterns (runtime-scaffolding tags, OpenAI special tokens, chat role markers, `trustedCommands` lines, "when agent is asked" rules), stacks a pattern-specific warning identifying which pattern matched and at what offset, and appends an entry to `ai/scratchpads/injection-guard.log` for later tuning. Content is never stripped; the minion still sees the full fetched text. The hook is wired in [`ai/swarm/bash-allowlist.json`](bash-allowlist.json) under `hooks.PostToolUse` alongside the allowlist; copy both blocks into a local `.claude/settings.json` to pick it up.

This layer is a **directive, not a fence**. PostToolUse `additionalContext` is prepended to the agent's view of the tool result; it cannot replace, wrap, or rewrite the bytes the model already sees. The minion reads the warning and the raw fetched text in the same context window. Recent research puts a number on the limit: *The Attacker Moves Second* (Debenedetti et al., Nov 2025) reports that even genuine Spotlighting fences fall to roughly 99% attack success rate under adaptive attacks, so a directive-only layer is strictly weaker than that already-weak ceiling. Treat this hook as defence-in-depth that catches the careless and reduces incidental drift; the architectural fix that delivers actual content fencing (an interception point that can mutate `tool_response` before the agent sees it) is tracked in SH-337.
