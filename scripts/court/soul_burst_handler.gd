...
const MOTE_SCENE: PackedScene = preload("res://scenes/effects/burst_soul_mote.tscn")
...
## Court assigns this once the paddle is spawned, since the paddle has no scene node.
var player_catcher: SoulCatcher
...
func release_burst(ball: Ball, payout: int) -> void:
	...
	for i in values.size():
		...
		var mote: BurstSoulMote = MOTE_SCENE.instantiate()
		mote.soul_value = values[i]
		mote.landed.connect(_on_mote_landed)
		add_child(mote)
		mote.global_position = ball.global_position
		mote.burst_toward(player_catcher, Vector2.RIGHT.rotated(angle_step * i))
