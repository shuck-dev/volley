...
## Injects a ball_tracker so the rack can source STORED-ball art from the registry. Test seam.
func configure_ball_tracker(tracker: Node) -> void:
...
func _registered_ball_for(ball_key: String) -> Ball:
	var active_ball_tracker: Node = ball_tracker if ball_tracker != null else BallTracker
	if active_ball_tracker == null:
		return null
	# A second stored ball can be left untracked by the ball_tracker's one-shot reconcile;
	# back-fill it here so every rendered stored slot is backed by a live, grabbable ball.
	return active_ball_tracker.create_ball_from_key(ball_key)
